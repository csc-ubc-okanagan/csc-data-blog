...
If we want to create an object that has different data types, a matrix
or vector won’t work because `R` will force all items to be of the same
type.
...
If we want a list that has a table-like structure (which will likely be
the case for a lot of the data you use in `R`), we can use a data frame.
...
The tidyverse set of packages provides a “fancy data frame” that does
not recycle elements (unless they are a single value), and allows you to
reference other columns you previously created:
