...
- **The SD for each dependent variable should be extracted**: In cases where other types of dispersion (SE, IQR, 95% CI, or range) are reported, they should be extracted and can be converted to SD as follows:
  ...
  - **Converting from SE)** SD = SE * $\sqrt{n}$, where n is the number of individuals.
  - **Converting from IQR)** SD = (upper limit - lower limit)/1.35.
  - **Converting from 95% Confidence Interval (CI))** ((Upper CI - Lower CI)/3.92) * $\sqrt{n}$, where n is the number of individuals.
  - **Converting from Range)** SD = (upper limit - lower limit)/4.
...
> - **7) Health status of study participants** The study may be conducted on healthy individuals or individuals with a specific disease. This is usually reported in the article title or the first paragraph of the Methods section.
...
The final topic to discuss in this part is whether the included articles have the **necessary quality** to perform a meta-analysis. To what extent do the included articles exhibit methodological errors? This assessment provides an overall view of the quality of the included articles. Previously, the Jadad tool was used for this purpose, but currently, the **Cochrane tool** is used. This tool is one of the most valid tools for assessing quality. It evaluates the quality of an article using seven factors, which assess the biases that may occur in a study and thereby evaluate its overall quality. We will not go into details of this tool as it is beyond the scope of this series, but you can find more information in the [Cochrane Handbook](https://training.cochrane.org/handbook) by Julian P. T. Higgins and Sally Green.
